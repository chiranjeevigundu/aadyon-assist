...
async function sendMessage() {
    ...
    domHist.insertAdjacentHTML('beforeend', renderMessage({role: "user", content: text}));
    scrollToBottom();
    
    // Setup assistant response bubble for streaming
    const bubbleId = "ast_" + Date.now();
    domHist.insertAdjacentHTML('beforeend', `
    ...
    try {
                    ...
                    if (line.startsWith("data: ")) {
                        ...
                        try {
                            ...
                            if (data.tool) {
                                ...
                                document.getElementById("container_" + bubbleId).insertAdjacentHTML('beforebegin', toolHtml);
                            }
        ...
        const bubble = document.getElementById(bubbleId);
        if (bubble) bubble.innerHTML = `<span style="color:var(--red)">Failed to send message.</span>`;
    }

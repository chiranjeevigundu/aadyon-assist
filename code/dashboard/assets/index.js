...
async function load() {
	...
	const dlRows = dls
      ...
      <td>${esc(x.blocked_on || "")}</td></tr>`,
		...
		.join("");
	app.append(
		$(`<div class="card"><h2>Deadlines</h2>
    ${dls.length ? `<table><thead><tr><th>Item</th><th>Due</th><th>Left</th><th>Status</th><th>Blocked on</th></tr></thead><tbody>${dlRows}</tbody></table>` : '<div class="empty">No deadlines.</div>'}</div>`),
	);
	...
	const section = (title, rows, head, body) =>
		`<div class="card"><h2>${title}</h2>${rows.length ? `<table><thead>${head}</thead><tbody>${body}</tbody></table>` : '<div class="empty">Nothing yet — add via the API.</div>'}</div>`;
	...
	wrap.append(
		$(
			section(
				"Bills",
				d.bills || [],
				'<tr><th>Name</th><th class="num">Amount</th><th>Due</th></tr>',
				(d.bills || [])
					.map(
						(b) =>
	...
	wrap.append(
		$(
			section(
				"Subscriptions",
				d.subscriptions || [],
				'<tr><th>Name</th><th class="num">Amount</th><th>Renews</th></tr>',
				(d.subscriptions || [])
					.map(
						(s) =>
							`<tr><td>${esc(s.name)}</td><td class="num">${money2(s.amount)}</td><td>${esc(s.renews_on || s.billing_cycle)}</td></tr>`,
					)
					.join(""),
			),
		),

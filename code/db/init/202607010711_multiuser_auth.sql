...
--   docker compose exec -T db psql -U aadyon -d aadyon_assist < code/db/init/202607010711_multiuser_auth.sql
--
...
--
-- NOTE on a fresh empty-volume boot: seed files (02_seed / 99_seed_local) must
-- assign user_id going forward. On the live DB this migration is applied manually
-- after data already exists, which is the common path (see AGENTS.md gotchas).
...
BEGIN
  ...
  SELECT p_user, v.name, v.dimension, v.mission FROM (VALUES
    ...
    ('Growth',      'goal',      'Make measurable progress toward the before-30 goals.')
  ...
  FROM (VALUES
    ...
    ('Head of Immigration','Head of Immigration','Immigration',
      'You lead Immigration. Track visa/OPT/STEM deadlines and blockers. propose_action for filings/emails (human approves).'),
    ('Head of Career','Head of Career','Career',
      'You lead Career. Assess pipeline and the income gap; recommend concrete next steps. propose_action for outreach.'),
    ('Head of Growth','Head of Growth','Growth',
      'You lead Growth. Track goals vs the before-30 timeline; recommend focus. propose_action for commitments.')

...
def test_signup_is_open_to_everyone(monkeypatch):
    """Signup takes no invite code and never consults one. Isolation between
    accounts is Postgres RLS's job, not a registration gate's."""
...
def test_the_invite_gate_is_gone_for_good(monkeypatch):
    """Guards against the gate being reintroduced by accident: no invite helper
    should exist on the auth service, and an invite_code in the payload is simply
    ignored rather than validated."""
    from app.routers import auth as auth_router
    from app.services import auth as auth_service

    for gone in ("consume_invite", "mark_invite_used", "create_invite", "has_any_user"):
        assert not hasattr(auth_service, gone), f"{gone} came back"

    seen = {}
    monkeypatch.setattr(
        auth_router.auth, "create_user", lambda e, p, d=None: seen.setdefault("u", {"id": "u1", "email": e})
    )
    monkeypatch.setattr(auth_router.auth, "make_token", lambda uid: "tok")
    monkeypatch.setattr(auth_router.auth, "get_user", lambda uid: {"id": "u1", "email": "a@b.c"})
    monkeypatch.setattr(auth_router, "_send_verification", lambda u: None)
    ...
    out = auth_router.signup(
        {"email": "a@b.c", "password": "pw", "invite_code": "whatever"}, request=None
    )
    assert out["token"] == "tok"
